...
public class PgAuditLogRepo(IDbConnectionProvider con) : IAuditLogRepo
{
	...
	public async Task<AuditLogDbm> Create(CreateAuditLogDbo ob)
	{
		...
		// Create command
		const string cmd = $"INSERT INTO audit.log (timestamp, table_name, action_type, row_id, user_id, previous_state, effected, comment) " +
		                   $"VALUES (@Timestamp, @TableName, @ActionType, @RowId, @UserId, @PreviousState, @Effected, @Comment) RETURNING *;";
